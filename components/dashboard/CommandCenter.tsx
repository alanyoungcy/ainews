...
export function CommandCenter({ trendRadar }: { trendRadar: TrendRadarFeed }) {
  ...
  async function rankWithJev() {
    ...
    try {
      ...
      const result = await response.json() as { scores?: JevTriageScore[]; model?: string; dedupe?: { uniqueItemIds?: string[]; duplicateCount?: number } };
  ...
  const visibleStories = useMemo(() => queueStories.filter((story) => {
    ...
    return matchesQuery && matchesFilter && matchesHistory;
  }), [dispatchedStories, queueStories, query, signalFilter, storyHistoryFilter]);
