...
export function CommandCenter({ trendRadar }: { trendRadar: TrendRadarFeed }) {
  ...
  return <>
    ...
    <div className="triage-page">
      <section className="triage-heading"><div><h1>Automated Source Ingestion &amp; Content Triage</h1><p>Continuous multi-feed ingestion from TrendRadar with AI relevance scoring, Capco context, and human approval before editorial synthesis.</p></div><div className="triage-heading-actions"><button className="button ghost small" onClick={syncLatestFeed} disabled={syncing}><Icon name="refresh" size={15} /> {syncing ? "Syncing latest…" : "Sync latest TrendRadar"}</button><div className="parser-health"><Icon name="check" size={15} /> Parsers: 100% OK</div></div></section>
      <section className="kpi-grid"><div className="kpi-card"><div><span>Ingested stories</span><Icon name="rss" size={17} /></div><strong>{feed.source.totalItems}</strong><small>{feed.source.rssItems} RSS · {feed.source.hotListItems} ranked signals</small></div><div className="kpi-card"><div><span>Fact grounding confidence</span><Icon name="check" size={17} /></div><strong>{aiBrief ? "82%" : "—"}</strong><small>{aiBrief ? `${aiBrief.meta.groundedSources} source groups grounded` : "Run synthesis to score claims"}</small></div><div className="kpi-card"><div><span>Edition curation progress</span><Icon name="layers" size={17} /></div><strong>{includedIds.length}<em> / 8</em></strong><div className="progress-track"><i style={{ width: `${Math.min(100, includedIds.length / 8 * 100)}%` }} /></div><small>Target: 8 stories · {Math.round(Math.min(100, includedIds.length / 8 * 100))}% quota</small></div><div className="kpi-card"><div><span>Pipeline status</span><Icon name="refresh" size={17} /></div><strong>{syncing ? "Syncing" : "Ready"}</strong><small>{feed.source.syncedAt ? `Snapshot synced ${new Date(feed.source.syncedAt).toLocaleString()}` : "Awaiting first sync"}</small></div></section>
    ...
    <div className="handoff-bar"><div><strong>{includedIds.length} of 8 stories confirmed</strong><span>for Week 42 editorial batch</span><small>TrendRadar snapshot · {feed.source.totalItems} items · {providerLabel}</small></div><div><button className="button utility">Save draft repository</button><button className="button dark" onClick={runAISynthesis} disabled={synthesizing}>{synthesizing ? "Synthesizing…" : aiBrief ? "Regenerate AI brief" : "Advance to Stage 02"}<Icon name="arrow" size={16} /></button></div></div>
