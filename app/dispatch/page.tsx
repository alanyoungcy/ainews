...
export default function DispatchPage() {
  ...
  const contextBlocks = useMemo(() => handoff ? [{ label: "01", title: "Locked story set", body: `${dispatchStories.length} Gate 1 approved stories queued for this edition.` }, { label: "02", title: "Capco PoV", body: editionPerspective }, { label: "03", title: "Visual specification", body: noVisual ? "No infographic requested" : `${handoff.visualStyle.name} · ${handoff.layout.name}` }, ...dispatchStories.map((story, index) => ({ label: `S${String(index + 1).padStart(2, "0")}`, title: story.title, body: `Source: ${story.source} · Reported fact: ${story.fact} · Capco PoV: ${story.capcoImplication}` }))] : [], [dispatchStories, editionPerspective, handoff, noVisual]);
