...
export default function DispatchPage() {
  ...
  useEffect(() => {
    ...
    async function restore() {
      ...
      try { const response = await fetch("/api/editorial-handoff?stage=stage03", { cache: "no-store" }); if (response.ok) { const payload = await response.json() as { handoff?: DispatchHandoff | null }; if (payload.handoff?.selectedStory?.title) local = chooseLatestHandoff(local, payload.handoff); } } catch { /* local remains available */ }
      if (cancelled) return;
  ...
  const editionTitle = selectedStory?.title ?? handoff?.infographic?.title ?? "Awaiting approved edition";
