...
export default function InfographicPage() {
  ...
  function applyHandoff(next: Stage02Handoff) { setHandoff(next); setAiBrief(next.brief); setSelectedStoryId(next.selectedStoryId ?? next.selectedStory?.id ?? "ai-story-1"); setBriefStatus(next.brief.meta.provider === "fallback" ? "fallback" : "ready"); setBriefError(next.brief.meta.providerError ?? null); setPrompt((current) => next.notes && !current.includes(next.notes) ? `${current}\nEditorial note: ${next.notes}` : current); logActivity("Stage 02 approved bundle restored · story, summary, PoV, and sources linked"); }

  useEffect(() => {
    let cancelled = false;
    async function restore() {
      let local = readLocalStage02();
      try { const response = await fetch("/api/editorial-handoff?stage=stage02", { cache: "no-store" }); if (response.ok) { const payload = await response.json() as { handoff?: Stage02Handoff | null }; if (payload.handoff) local = payload.handoff; } } catch { /* local remains available */ }
      if (cancelled) return;
  ...
  function confirmStory() { if (!selectedStory) return; setConfirmed(true); logActivity(`Story confirmed · ${selectedStory.title}`); }

  async function runOcrDetection() {
    if (!selectedStory) return;
    setOcrStatus("running");
    logActivity("OCR detection running · isolating headline, summary, PoV, metrics, and source footer");
    try {
      const response = await fetch("/api/ocr", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ textPlan, metrics, source: selectedStory.source }) });
      const result = response.ok ? await response.json() as { overlays?: Overlay[] } : null;
      if (result?.overlays?.length) setOverlays(result.overlays);
      else setOverlays([{ id: "headline", label: "Headline", value: textPlan.headline, x: 7, y: 8, width: 70, kind: "headline" }, { id: "summary", label: "Summary", value: textPlan.summary, x: 7, y: 34, width: 42, kind: "summary" }, { id: "pov", label: "Capco PoV", value: textPlan.perspective, x: 7, y: 56, width: 48, kind: "pov" }, ...metrics.map((value, index) => ({ id: `metric-${index}`, label: `Metric ${index + 1}`, value, x: 62, y: 38 + index * 12, width: 30, kind: "metric" as const })), { id: "source", label: "Source", value: selectedStory.source, x: 7, y: 92, width: 50, kind: "source" }]);
      setOcrStatus("ready");
      logActivity("OCR boxes ready · copy is editable on the visual canvas");
    } catch { setOcrStatus("ready"); logActivity("OCR service unavailable · deterministic editable text boxes restored"); }
  }
  ...
  function persistStage03Handoff(nextArtworkUrl = artworkUrl) {
    if (!selectedStory) return;
    const next: Stage03Handoff = { version: 2, savedAt: new Date().toISOString(), brief: aiBrief, stage02: handoff, stories: approvedStories, selectedStory, visualStyle: { id: selectedVisualStyle.id, name: selectedVisualStyle.name }, layout: { id: selectedArchetype.id, name: selectedArchetype.name }, infographic, metrics, overlays, artworkUrl: nextArtworkUrl, referenceImage, watermark, legalScrim, status: signedOff ? "confirmed" : "draft" };
  ...
  async function generateArtwork() {
    ...
    try {
      ...
      if (result.output) { setArtworkUrl(result.output); setArtworkModel(result.model ?? "configured image model"); setArtworkStatus(result.cached ? "cached" : "generated"); logActivity(result.cached ? "Artwork restored from visual memory" : "Image model returned a complete infographic"); await runOcrDetection(); persistStage03Handoff(result.output); }
  ...
  useEffect(() => { if (artworkUrl && selectedStory) persistStage03Handoff(); }, [artworkUrl, selectedStoryId, visualStyle, archetype, metrics, overlays, watermark, legalScrim, referenceImage, signedOff]);
  ...
  return <AppShell><div className="stage-page infographic-stage studio-stage">
    ...
    <div className="studio-grid"><aside className="studio-rail panel"><div className="studio-panel-heading"><div><span className="section-kicker">01 · Approved story bundle</span><h2>Story to visualize</h2></div><span className={`lock-label ${handoff ? "locked" : ""}`}>{handoff ? "Locked" : "Waiting"}</span></div><div className="story-picker">{approvedStories.length ? approvedStories.map((story, index) => <button key={story.id} className={`story-picker-row ${selectedStory?.id === story.id ? "selected" : ""}`} onClick={() => { setSelectedStoryId(story.id); setConfirmed(false); }}><span className="story-picker-index">{String(index + 1).padStart(2, "0")}</span><span><strong>{story.title}</strong><small>{story.source} · {story.confidence}% grounding</small></span><Icon name="chevron" size={15} /></button>) : <div className="empty-studio">Return to Stage 02 and approve a story first.</div>}</div>{selectedStory && <div className="confirmed-story"><span>Approved summary</span><strong>{selectedStory.title}</strong><p>{selectedStory.fact}</p><div className="handoff-detail"><span>Capco PoV</span><p>{selectedStory.capcoImplication}</p></div><button className={`button ${confirmed ? "ghost" : "primary"} full`} onClick={confirmStory}><Icon name="check" size={14} />{confirmed ? "Story confirmed" : "Confirm selected story"}</button></div>}
      ...
      <main className="studio-canvas-column"><div className="studio-canvas-wrap"><div className={`studio-canvas ${artworkUrl ? "has-artwork" : ""} visual-${visualStyle} layout-${archetype}`} style={artworkUrl ? { backgroundImage: `linear-gradient(90deg, rgba(11,19,43,.88), rgba(11,19,43,.24)), url(${artworkUrl})` } : undefined}><div className="studio-canvas-scrim" /><div className="studio-canvas-topline"><span>CAPCO / AI INTELLIGENCE</span><span>{selectedVisualStyle.name} · {selectedArchetype.name}</span></div><div className="studio-canvas-copy"><span className="studio-canvas-kicker">OCR EDITABLE CANVAS</span><h2>{textPlan.headline}</h2><p>{textPlan.summary}</p></div><div className="studio-info-band"><span>APPROVED SOURCE STORY</span><strong>{selectedStory?.title ?? "Waiting for Stage 02"}</strong><p>{selectedStory?.fact ?? "The approved summary will appear here."}</p></div><div className="studio-pin pin-one"><b>PIN [T-01]</b><span>Signal intake</span></div><div className="studio-pin pin-two"><b>PIN [T-02]</b><span>Decision gate</span></div><div className="studio-pin pin-three"><b>PIN [T-03]</b><span>Control evidence</span></div><div className="studio-metric-stack">{metrics.map((metric) => <span key={metric}>{metric}</span>)}</div><div className="studio-key-finding"><span>CAPCO PERSPECTIVE</span><strong>{textPlan.perspective}</strong></div>{overlays.map((overlay) => <label key={overlay.id} className={`ocr-overlay ocr-${overlay.kind}`} style={{ left: `${overlay.x}%`, top: `${overlay.y}%`, width: `${overlay.width}%` }}><span>{overlay.label}</span><textarea value={overlay.value} onChange={(event) => updateOverlay(overlay.id, event.target.value)} rows={overlay.kind === "headline" ? 2 : 3} /></label>)}<div className="studio-canvas-footer"><span>{watermark ? "CAPCO INTELLIGENCE STUDIO" : ""}</span><span>{legalScrim ? "Source-grounded · legal review required" : "Draft visual · legal review required"}</span></div></div></div><div className="studio-canvas-status"><span><i className={`status-dot ${artworkUrl ? "green" : "orange"}`} />{artworkLabel}</span><span>{ocrStatus === "ready" ? "OCR text boxes are editable" : "Generate artwork, then detect text"}</span><button className="button utility" onClick={() => void runOcrDetection()} disabled={!selectedStory || ocrStatus === "running"}>{ocrStatus === "running" ? "Detecting text…" : "Run OCR detection"}</button></div><div className="activity-timeline"><div className="activity-heading"><span>Generation activity</span><small>Story and asset handoff log</small></div>{activity.map((item, index) => <div key={`${item}-${index}`} className="activity-row"><i className={index === activity.length - 1 && (artworkStatus === "generating" || ocrStatus === "running") ? "activity-spinner" : "activity-dot"} /><span>{item}</span></div>)}</div></main>
      <aside className="studio-inspector panel"><div className="studio-panel-heading"><div><span className="section-kicker">06 · OCR &amp; source locks</span><h2>Final text controls</h2></div><span className="lock-label locked">Fact-locked</span></div>{selectedStory ? <><div className="source-lock-card"><span>Confirmed provenance</span><strong>{selectedStory.source}</strong><small>{selectedStory.url ?? "TrendRadar source record"}</small><p>{selectedStory.fact}</p></div><div className="source-lock-card capco"><span>Capco PoV</span><p>{selectedStory.capcoImplication}</p></div></> : <div className="empty-studio">Approve a story in Stage 02 to unlock source locks.</div>}<div className="studio-control-block"><span className="studio-control-label">Headline and metrics</span>{metrics.map((metric, index) => <label className="studio-field metric-field" key={`metric-${index}`}><span>Metric {String(index + 1).padStart(2, "0")}</span><input value={metric} onChange={(event) => setMetrics((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} /></label>)}</div><div className="studio-control-block"><span className="studio-control-label">Detected OCR boxes</span><div className="ocr-list">{overlays.length ? overlays.map((overlay) => <label key={overlay.id}><span>{overlay.label}</span><input value={overlay.value} onChange={(event) => updateOverlay(overlay.id, event.target.value)} /></label>) : <small>Run OCR detection after generation to create editable bounding boxes.</small>}</div></div><div className="studio-control-block"><span className="studio-control-label">Brand guardrails</span><button className="guardrail-row" onClick={() => setWatermark((value) => !value)}><span><strong>Institutional watermark</strong><small>Capco Intelligence Studio</small></span><span className={`toggle ${watermark ? "on" : ""}`}><i /></span></button><button className="guardrail-row" onClick={() => setLegalScrim((value) => !value)}><span><strong>Legal-clearance scrim</strong><small>Source footer remains visible</small></span><span className={`toggle ${legalScrim ? "on" : ""}`}><i /></span></button></div><div className="signoff-card"><span>HITL Gate 2</span><strong>{signedOff ? "Lead consultant sign-off recorded" : "Review image and OCR overlays"}</strong><small>{signedOff ? "Stage 04 publishing bundle is ready." : "Confirm the visual after editing detected text boxes."}</small><button className="button dark full" onClick={() => { setSignedOff(true); persistStage03Handoff(); }} disabled={!artworkUrl || !overlays.length || signedOff}><Icon name="check" size={15} />{signedOff ? "Visual confirmed" : "Confirm visual"}</button></div></aside>
    </div>
    <div className="handoff-bar"><div><strong>{signedOff ? "Visual confirmed" : artworkUrl ? "Complete infographic ready for review" : "Visual curation in progress"}</strong><span>{selectedStory ? `Selected story · ${selectedStory.source}` : "Stage 02 story handoff required"}</span><small>{signedOff ? "Stage 04 publishing bundle unlocked" : "Story, approved copy, style, layout, OCR overlays, and artwork stay linked"}</small></div><div><button className="button utility" onClick={() => { persistStage03Handoff(); setSaved(true); }}>{saved ? "Saved to bundle" : "Save visual bundle"}</button><button className="button dark" disabled={!signedOff} onClick={() => { persistStage03Handoff(); window.location.href = "/dispatch"; }}>Advance to Stage 04 <Icon name="arrow" size={16} /></button></div></div>
  </div></AppShell>;
}
