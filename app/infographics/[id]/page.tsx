...
export default function InfographicPage() {
  ...
  function persistStage03Handoff(nextArtworkUrl = artworkUrl, nextOverlays = overlays) {
  ...
  async function generateArtwork() {
    ...
    try {
      const response = await fetch("/api/generate-art", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brief: aiBrief.brief, story: selectedStory, archetype: selectedArchetype.name, layout: selectedArchetype.name, visualStyle: selectedVisualStyle.name, prompt, seed, includeText: false, referenceImage, textPlan: null }) });
      ...
      if (result.output) { setArtworkUrl(result.output); setArtworkModel(result.model ?? "configured image model"); setArtworkStatus(result.cached ? "cached" : "generated"); setOverlays([]); setOcrStatus("idle"); logActivity(result.cached ? "Image-only artwork restored from visual memory" : "Image model returned an image-only visual"); persistStage03Handoff(result.output, []); }
  ...
  return <AppShell><div className="stage-page infographic-stage studio-stage">
    <div className="studio-pipeline-shell"><div className="studio-pipeline-brand"><span className="studio-pipeline-mark">C</span><span><strong>CAPCO INTELLIGENCE STUDIO</strong><small>Stage 02 approval → visual curation → image-only artwork</small></span></div><div className="studio-pipeline-steps"><span className="done"><b>01</b> Triage <small>Done</small></span><i /><span className="done"><b>02</b> Review <small>Approved</small></span><i /><span className="active"><b>03</b> Visual curation <small>In progress</small></span><i /><span><b>04</b> Publish <small>Pending</small></span></div></div>
    <section className="stage-heading studio-stage-heading"><div><span className="section-kicker">Visual curation &amp; image generation</span><h1>Build the visual communication</h1><p>Choose a visual language and generate an image-only asset from the approved editorial bundle. Text and OCR overlays are temporarily disabled.</p></div><div className="stage-heading-actions"><span className="status-chip"><span className={`status-dot ${signedOff ? "green" : briefStatus === "error" ? "red" : "orange"}`} />{signedOff ? "Visual confirmed" : briefLabel}</span><button className="button dark" onClick={() => { setSignedOff(true); persistStage03Handoff(); }} disabled={!artworkUrl || signedOff}><Icon name="check" size={16} />{signedOff ? "Visual confirmed" : "Confirm visual"}</button></div></section>
    <div className="stage-progress ready" role="status" aria-live="polite"><span className="stage-progress-icon"><Icon name="check" size={15} /></span><div><strong>{briefLabel}</strong><span>{selectedStory ? `${selectedStory.title} · ${selectedStory.source}` : "Awaiting Stage 02 approval"}</span>{briefError && <small>{briefError}</small>}</div><div className="stage-progress-actions"><span className="stage-progress-step">{approvedStories.length} approved stories · image-only mode</span></div></div>
